dingtalk-agent（/Users/yushui/Desktop/dingtalk-agent）：钉钉群报修工单系统，Python 3.14 + asyncio + SQLite(WAL) + LangGraph 1.2，~27k 行自研代码。运行入口 main.py，业务主干 pipeline.py(1799 行)/db.py(1874 行)，数据真相源 data/tickets.db（gitignore），Agent checkpoint 为 data/langgraph-checkpoints.sqlite。生产/测试群配置为脱敏示例（测试群（示例）），依赖本机 dws CLI。文档主索引 计划书.md(2533 行) + docs/。审查快照 2026-09-16：224+ 测试，8 个集成测试红（均为 test_pipeline_integration.py）。
§
